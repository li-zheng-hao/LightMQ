...
public class MongoProviderTests : IAsyncLifetime
{
    ...
    public async Task ClearOldMessagesAsync_ShouldRemoveOldMessages()
    {
        ...
        // Assert
        var deletedMessage = await _collection.Find(m => m.Id == oldMessage.Id).FirstOrDefaultAsync();
    ...
    public async Task PublishNewMessageAsync_WithTransaction_ShouldInsertMessage()
    {
        ...
        // Act
        await _mongoStorageProvider.PublishNewMessageAsync(message, clientSession, CancellationToken.None);
    ...
    public async Task ResetOutOfDateMessagesAsync_ShouldUpdateOutOfDateMessages()
    {
        ...
        // Act
        await _mongoStorageProvider.ResetOutOfDateMessagesAsync(CancellationToken.None);

        // Assert
        var updatedMessage = await _collection.Find(m => m.Id == oldMessage.Id).FirstOrDefaultAsync();
        Assert.NotNull(updatedMessage);
        Assert.Equal(MessageStatus.Waiting, updatedMessage.Status); // 验证状态是否更新为 Waiting
    }
    ...
    public async Task PollNewMessageAsync_ShouldReturnAndUpdateMessageStatus()
    {
        ...
        // Act
        var polledMessage = await _mongoStorageProvider.PollNewMessageAsync("PollTopic", CancellationToken.None);
    ...
    public async Task PollNewMessageAsync_WithQueue_ShouldReturnAndUpdateMessageStatus()
    {
        ...
        // Act
        var polledMessage =
            await _mongoStorageProvider.PollNewMessageAsync("PollTopic", "PollQueue", CancellationToken.None);
...
public async Task PublishNewMessagesAsync_ShouldInsertMultipleMessages()
{
    ...
    // Assert
    var insertedMessages = await _collection.Find(m => messages.Select(msg => msg.Id).Contains(m.Id)).ToListAsync();
