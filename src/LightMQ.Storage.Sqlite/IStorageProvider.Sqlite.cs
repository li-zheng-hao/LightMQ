...
public class SqliteStorageProvider : IStorageProvider
{
    ...
    public SqliteStorageProvider(IOptions<LightMQOptions> mqOptions, IOptions<SqliteOptions> dbOptions)
    {
    ...
    public async Task ClearOldMessagesAsync(CancellationToken cancellationToken = default)
    {
        ...
        await connection.ExecuteAsync(sql,
             new 
                { CreateTime=DateTime.Now.Subtract(_mqOptions.Value.MessageExpireDuration) });
    }
    ...
    public async Task ResetOutOfDateMessagesAsync(CancellationToken cancellationToken = default)
    {
        ...
        await connection.ExecuteAsync(sql,
             new 
            {
                ...
                ExecutableTime= DateTime.Now.Subtract(_mqOptions.Value.MessageTimeoutDuration),
    ...
    RETURNING Id, Topic, Data, CreateTime, Status, ExecutableTime, RetryCount, Header, Queue";
        var connection = new SqliteConnection(_dbOptions.Value.ConnectionString);
        await using var _ = connection.ConfigureAwait(false);
        return await connection.QueryFirstOrDefaultAsync<Message>(sql,
             new 
            {
...
GROUP BY Queue;";
        ...
        await using var _ = connection.ConfigureAwait(false);
        var queues=await connection.QueryAsync<string?>(sql, new
        {
            Topic = topic,
            StatusOrigin = MessageStatus.Waiting,
            ExecutableTime = DateTime.Now
    ...
    public async Task AckMessageAsync(Message currentMessage, CancellationToken stoppingToken = default)
    {
        ...
        await connection.ExecuteAsync(sql,
            new 
                { Status= MessageStatus.Success,  currentMessage.Id });
    }
