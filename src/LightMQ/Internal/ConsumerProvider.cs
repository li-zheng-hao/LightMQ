...
public class ConsumerProvider : IConsumerProvider
{
    ...
    {
        if (_options.Value.ConsumerAssembly == null)
            throw new ArgumentNullException(nameof(_options.Value.ConsumerAssembly));

        var consumersTypes = _options
            .Value.ConsumerAssembly!.ExportedTypes.Where(it =>
                typeof(IMessageConsumer).IsAssignableFrom(it)
            )
            .ToList();
        ...
        foreach (var consumersType in consumersTypes)
        {
            ...
            {
                _logger.LogWarning(
                    $"扫描到了{consumersType.FullName}，但是没有从ServiceProvider中获取到实例，跳过处理"
                );
                continue;
            }
            _consumerInfos.Add(
